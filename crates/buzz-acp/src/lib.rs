...
/// - `"Authentication required"` — Copilot CLI's ACP `session/new` error when
///   the CLI has not completed `copilot login`.
///
        ...
        if !removed_channels.contains(&batch.channel_id) {
            if matches!(
                ...
                let content = "⚠️ I couldn't process the last request: authentication failed. \
                    Please re-authenticate the CLI (e.g. run `claude /login`, `codex login`, or `copilot login`) \
                    and then re-send."
                    .to_string();
                spawn_failure_notice(rest_client, &batch, content);
...
mod error_outcome_emission_tests {
    ...
    #[test]
    fn is_auth_error_matches_copilot_authentication_required() {
        ...
        assert!(
            is_auth_error(&error),
            "Copilot ACP authentication failure must be classified as non-retryable"
        );
    }
