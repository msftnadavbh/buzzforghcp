...
fn default_agent_args(command: &str) -> Option<Vec<String>> {
    match normalize_agent_command_identity(command).as_str() {
        ...
        "copilot" => Some(vec!["--acp".to_string(), "--stdio".to_string()]),
...
mod tests {
    ...
    #[test]
    fn normalizes_copilot_args_to_native_acp_stdio() {
        assert_eq!(
            normalize_agent_args("copilot", Vec::new()),
            vec!["--acp", "--stdio"]
        );
        assert_eq!(
            normalize_agent_args(r"C:\Program Files\GitHub Copilot\copilot.exe", Vec::new()),
            vec!["--acp", "--stdio"]
        );
    }
