...
Supports any agent that speaks [ACP](https://agentclientprotocol.com/) over stdio, including **goose**, **codex** (via [codex-acp](https://github.com/agentclientprotocol/codex-acp)), **claude code** (via [claude-agent-acp](https://github.com/agentclientprotocol/claude-agent-acp)), and **GitHub Copilot CLI**.
...
GitHub Copilot CLI has a native ACP stdio server. Install it, authenticate the
CLI, then select **GitHub Copilot** from Buzz Desktop's runtime catalog. An
active Copilot subscription is required, and organization policy must allow
Copilot CLI. See the [all-platform installation guide](../../docs/github-copilot-cli.md)
for macOS, Windows, Linux, and WSL instructions.
...
Copilot ACP support is currently public preview. Buzz auto-approves each ACP
tool request once, matching other managed headless runtimes. Run agents only in
workspaces you trust. If your enterprise uses a data-residency host, authenticate
before starting Buzz with `copilot login --host https://example.ghe.com`.
...
**Tier-1 — compiled-in runtimes** (Goose, Claude Code, Codex, GitHub Copilot, Buzz Agent): have first-class discovery and installation metadata. Their IDs (`goose`, `claude`, `codex`, `copilot`, `buzz-agent`) are reserved and cannot be overridden. Copilot authentication is completed separately with `copilot login` because its CLI exposes no non-interactive login-status command.
...
The built-in `BUILTIN_IDS` set (`goose`, `claude`, `codex`, `copilot`, `buzz-agent`, and all current preset ids) is the reserved namespace; every other id is available for custom harnesses.
